...
const hasConfigId = computed(() => Boolean(props.configId));

const isDisabled = computed(() => !isReady.value || isLoading.value || !hasConfigId.value);

const buttonLabel = computed(() => {
  if (!hasConfigId.value) return "Missing config";
...
function launchSignup(): void {
  if (!FB.value || !isReady.value || !props.configId) return;
  if (import.meta.env.DEV) {
    console.info("[FacebookSignUp] Launching Embedded SignUp with config ID:", props.configId);
  }
  _sessionData = {};
    ...
    FB.value.login(
      (response) => {
        ...
        if (!code && !accessToken) {
          emit(
            "error",
            new Error("[SignupLauncher] No auth code or token in FB.login() response.")
          );
          return;
        }
...
<template>
  <div class="flex flex-col items-center gap-y-8">
    ...
    <p v-if="!hasConfigId" class="text-sm text-amber-600" role="status">
      Set <code class="rounded bg-neutral-200 px-1">VITE_META_CONFIG_ID</code> in your
      <code class="rounded bg-neutral-200 px-1">.env</code> to enable.
    </p>
